...
echo "Starting API (3001) and Next.js (${WEB_PORT}) — no migrate/seed…"
exec pnpm exec concurrently -k -n api,web -c blue,green \
  ...
  "cd \"$GREENQUOTE_ROOT/frontend\" && PORT=\"$WEB_PORT\" AUTH_SECRET=\"$AUTH_SECRET\" AUTH_URL=\"$AUTH_URL\" API_URL=\"$API_URL\" pnpm run dev"
